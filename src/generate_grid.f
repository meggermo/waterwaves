...
C
      ...
      REAL   (KIND=RK) CRD (2, 2, *)     ! grid point coordinates (OUT)
C
...
C        Generate the grid depending on the value of GRT
         CALL GENERATE_GRID_NW (NGP, GRT, GRID_PAR, CRD (1, 1, IGP))
...
C
  ...
  101 FORMAT (1X, 'NETWORK', I3, ', NGP =', I4)
      ...
      SUBROUTINE GENERATE_GRID_NW (NGP, GRT, GRID_PAR, CRD)
C ---------------------------------------------------------------------------
C     Generates the initial grid for a network
C ---------------------------------------------------------------------------
      IMPLICIT NONE
      INCLUDE 'knd_params.inc'
...
C
      INTEGER(KIND=IK) GRT
      REAL   (KIND=RK) GRID_PAR (N_GRP)
      REAL   (KIND=RK) CRD (2, 2, NGP)     ! grid point coordinates (OUT)
C
      IF (GRT .EQ. GRD_LINE) THEN
        CALL LINE_GRID (NGP, GRID_PAR, CRD)
      ELSE IF (GRT .EQ. GRD_STRETCHED) THEN
        CALL STRETCHED_GRID (NGP, GRID_PAR, CRD)
      ELSE IF (GRT .EQ. GRD_CIRCLESECT) THEN
        CALL CIRCLESECT_GRID (NGP, GRID_PAR, CRD)
      ELSE IF (GRT .EQ. GRD_CUBIC) THEN
        CALL CUBIC_GRID (NGP, GRID_PAR, CRD)
      ELSE
